...
final class OAuth2LoginBeanDefinitionParser implements BeanDefinitionParser {
	...
	private static final String ATT_ACCESS_TOKEN_RESPONSE_CLIENT_REF = "tokenendpoint-access-token-response-client-ref";
	private static final String ATT_OAUTH2_USER_SERVICE_REF = "userinfo-user-service-ref";
	private static final String ATT_OAUTH2_OIDC_USER_SERVICE_REF = "userinfo-oidc-user-service-ref";
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		oauth2LoginAuthenticationProvider = BeanDefinitionBuilder
		...
		oauth2AuthorizationRequestRedirectFilter = BeanDefinitionBuilder
				.rootBeanDefinition(OAuth2AuthorizationRequestRedirectFilter.class)
				.addConstructorArgValue(new RuntimeBeanReference(ClientRegistrationRepository.class))
				.addPropertyValue("authorizationRequestRepository", oauth2AuthRequestRepository).getBeanDefinition();
	...
	private BeanDefinition retrieveOAuth2OidcAuthProvider(Element element,
		...
		if (oidcAuthenticationProviderEnabled) {
			...
			// TODO jwtDecoderFactory will throw bean not found, how to construct property
			// value if bean not found then use default decoder?
			oauth2OidcAuthProvider = BeanDefinitionBuilder.rootBeanDefinition(
					...
					.addPropertyValue("jwtDecoderFactory",
							new RuntimeBeanReference("org.springframework.security.oauth2.jwt.JwtDecoderFactory"))
					.getBeanDefinition();
		} else {
			// TODO unable to return default checker because
			// OidcAuthenticationRequestChecker was private
			oauth2OidcAuthProvider = BeanDefinitionBuilder.rootBeanDefinition(
	...
	private BeanMetadataElement retrieveOAuth2OidcUserService(Element element) {
		...
		String oauth2UserServiceRef = element.getAttribute(ATT_OAUTH2_OIDC_USER_SERVICE_REF);
	...
	private BeanMetadataElement retrieveOAuth2UserService(Element element) {
		...
		String oauth2UserServiceRef = element.getAttribute(ATT_OAUTH2_USER_SERVICE_REF);
		if (!StringUtils.isEmpty(oauth2UserServiceRef)) {
	...
	protected final RequestMatcher getAuthenticationEntryPointMatcher() {
